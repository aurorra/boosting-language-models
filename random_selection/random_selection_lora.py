"""
Script to randomly add synthetic data. Currently, we add the data once and do one finetuning run with lora.

Example usage:
python3 ablation_study_whole_pot_lora.py DiscoResearch/Llama3-German-8B data/boosting_V2/synthetic_data_verb_V3_100000 results/synthetic_data_verb_V3_100000_ablation/DiscoResearch/Llama3-German-8B data/boosting_V2/synthetic_data/combined_V3_100000_remaining_data.pickle
"""
...
from transformers import Trainer, TrainingArguments, AutoTokenizer, AutoModelForSequenceClassification, set_seed, \
    DataCollatorWithPadding, BitsAndBytesConfig, EarlyStoppingCallback
...
print(f'Parameters:\nmodel_ckpt = {model_ckpt}\ndata_path = {data_path}\noutput_path = {output_path}\n'
      f'synthetic_data_path = {synthetic_data_path}')
...
def load_data(path):
    ...
    # val_subset = get_balanced_subset(val_dataset, 10)
...
def tokenize_data(hf_dataset):
    ...
    tokenized_hf_dataset = hf_dataset.map(tokenize, batched=True, batch_size=20000, load_from_cache_file=False)
...
def convert_to_hf(dataset):
    """ Converts pandas DataFrame to a HuggingFace Dataset. """
...
def model_init(quantization_config):
    """
...
def training_loop(num_inits=1):
    """
    Trains the model after randomly adding synthetic data from a pickle file.
    """
    ...
    for current_run in range(run, num_inits):
        ...
        current_train_set_df = pd.concat([train_set.to_pandas(), synthetic_data], ignore_index=True)
        ...
        current_train_set_df = current_train_set_df.sample(frac=1, random_state=current_seed).reset_index(drop=True)
        # Re-tokenize the combined dataset for HuggingFace Trainer
        current_train_set = tokenize_data(convert_to_hf(current_train_set_df))
        print('New train dataset:')
        print(current_train_set)

        # init new model
        model = prepare_and_init_model()
        ...
        trainer.model.save_pretrained(f'{output_path}/init_{current_run}/{model_ckpt}-final')
        empty_cache()
        ...
        seconds = elapsed % 60
        print(f'Time elapsed: {hours} h, {minutes} min, {seconds:.1f} s')
        ...
        with open(f'{output_path}/init_{current_run}/results.csv', 'w') as f:
            ...
            f.write(f'number of samples in training set = {len(current_train_set)}\n')
            f.write(f'number of samples in validation set = {len(val_set)}\n')
            f.write(f'Time elapsed: {hours} h, {minutes} min, {seconds:.1f} s')
...
if __name__ == '__main__':
    ...
    if 'LeoLM/leo-mistral-hessianai-7b' in model_ckpt:
        ...
        if '_cap_' in data_path:
            syn_data_to_add = int((135056 - 50000) / 2)
        if '_verb_' in data_path:
            syn_data_to_add = int((120980 - 50000) / 2)
        if '_case_error_' in data_path:
            syn_data_to_add = int((188534 - 50000) / 2)
        if '_real_word_' in data_path:
            syn_data_to_add = int((293416 - 50000) / 2)
    ...
    if 'DiscoResearch/Llama3-German-8B' in model_ckpt:
        ...
        if '_cap_' in data_path:
            syn_data_to_add = int((133814 - 50000) / 2)
        if '_verb_' in data_path:
            syn_data_to_add = int((125496 - 50000) / 2)
        if '_case_error_' in data_path:
            syn_data_to_add = int((185558 - 50000) / 2)
        if '_real_word_' in data_path:
            syn_data_to_add = int((291498 - 50000) / 2)

    # hyperparameters for ablation
    n_inits = 1

    if syn_data_to_add == 0:
        print('No synthetic data to add!')
        exit()

    # create output path if it does not exist yet
    output_path = f'{output_path}/n_inits={n_inits}_num_samples={syn_data_to_add}'
    ...
    # Convert datasets
    train_set = tokenize_data(convert_to_hf(df_train))
    val_set = tokenize_data(convert_to_hf(df_val))
    test_set = tokenize_data(convert_to_hf(df_test))

    # Run the training loop
    training_loop(num_inits=n_inits)
