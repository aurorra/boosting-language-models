...
from transformers import Trainer, TrainingArguments, AutoTokenizer, AutoModelForSequenceClassification, set_seed, \
    BitsAndBytesConfig, EarlyStoppingCallback, DataCollatorWithPadding
...
def training_loop_with_boosting():
    """
    ...
    # load last checkpoint if given
    run, model_state_dict, optimizer_state_dict, trainer_state_dict, already_added, train_set_df = load_checkpoint(
        output_path)
    if run is None:
        run = 0
        already_added = set()
        # Sort model parameters by their name to enforce a consistent order.
        sorted_params = [param for _, param in sorted(model.named_parameters(), key=lambda item: item[0])]
        optimizer = torch.optim.AdamW(sorted_params, lr=learning_rate)
        ...
        # training set from previous boosting round which already includes additional data
        current_train_set = tokenize_data(convert_to_hf(train_set_df))
    ...
    for current_run in range(run, n_runs):
        ...
        trainer.model.save_pretrained(f'{output_path}/boosting_run_{current_run}/{model_ckpt}-final')
        empty_cache()
        ...
        build_confusion_matrix(val_predictions.predictions.argmax(-1), val_predictions.label_ids,
                               f'{output_path}/boosting_run_{current_run}', iteration=current_run + 1)
        ...
        if current_run == n_runs - 1:
            save_checkpoint(current_run + 1, model, trainer, optimizer, already_added, current_train_set, output_path)
            break
        ...
        if not synthetic_data.empty:
            ...
            train_set_df = pd.concat([current_train_set.to_pandas(), synthetic_data], ignore_index=True)
